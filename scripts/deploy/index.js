...
const { serverIp, user, targetDir, localTarDir, tarDirName, projectName } = config.deployConfig
let pwd
const getPwd = async () => {
...
const execute = () => {
  ...
  const params = [path.resolve(__dirname, './start.sh'), user, pwd, serverIp, targetDir, tarDirName, projectName]
  const result = spawn('expect', params, options)
