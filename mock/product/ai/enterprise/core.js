module.exports = {
  title: '核心功能',
  bg: '#F4F4F4',
  content: [
    {
      title: '信息资源管理',
      text: 'ERP 的提出与计算机技术的高度发展是分不开的，用户对系统有更大的主动性，作为计算机辅助管理所涉及的功能已远远超过 MRP II 的范围',
      icon: '/images/product/ai/enterprise/core/c1.svg',
      alt: '信息资源管理',
    },
    {
      title: '物资资源',
      text: 'ERP 是一种主要面向制造行业进行物质资源、资金资源和信息资源集成一体化管理的企业信息管理系统。ERP 是一个以管理会计 为核心可以提供跨',
      icon: '/images/product/ai/enterprise/core/c2.svg',
      alt: '物资资源',
    },
    {
      title: '财务资源管理',
      text: '重新构建企业的业务和信息流程及组织结构，使企业在市场竞争中有更大的能动性',
      icon: '/images/product/ai/enterprise/core/c3.svg',
      alt: '财务资源管理',
    },
    {
      title: '运输管理',
      text: '人们不仅可以把一个汉字的笔划或偏旁等单元组成一个组块，而且能把经常在一起出现的字或词组成组块单位来加以识别',
      icon: '/images/product/ai/enterprise/core/c4.svg',
      alt: '运输管理',
    },
    {
      title: '过程控制接口',
      text: 'SQL结构化查询语言、图形用户界面、4GL/CASE、窗口技术、人工智能、仿真技术等等',
      icon: '/images/product/ai/enterprise/core/c5.svg',
      alt: '过程控制接口',
    },
  ],
};
