module.exports = {
  title: '核心功能',
  bg: '#333333',
  content: [
    {
      title: '虚无法球',
      text: '卡萨丁向目标发射虚无法球，造成65/95/125/155/185(+0.7AP)魔法伤害并打断目标的技能引导',
      icon: '/images/product/dataserver/offline-develop/core/c1.svg',
      alt: '虚空之刃',
    },
    {
      title: '虚空之刃',
      text: '卡萨丁给他的虚空之刃充能，使他的下次普通攻击造成70/95/120/145/170(+0.8AP)额外魔法伤害',
      icon: '/images/product/dataserver/offline-develop/core/c2.svg',
      alt: '虚空之刃',
    },
    {
      title: '能量脉冲',
      text: '卡萨丁就可以使用能量脉冲，来对前方锥形范围内的敌人造成80/105/130/155/180(+0.8AP)魔法伤害',
      icon: '/images/product/dataserver/offline-develop/core/c3.svg',
      alt: '能量脉冲',
    },
    {
      title: '虚空行走',
      text: '卡萨丁传送到附近的目标区域，着陆时对身边的所有敌方单位造成80/100/120(+*)(+0.2*法术强度)魔法伤害',
      icon: '/images/product/dataserver/offline-develop/core/c4.svg',
      alt: '虚空行走',
    },
  ],
};
